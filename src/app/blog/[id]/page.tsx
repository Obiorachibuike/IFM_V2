...
export default function BlogDetailPage() {
  ...
  // Memoize lookup to prevent unnecessary re-runs
      ...
      <section className="relative h-[70vh] min-h-[600px] flex items-end pb-20 overflow-hidden">
        ...
        <div className="container relative z-20 mx-auto px-6 max-w-5xl">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <Link href="/blog" className="inline-flex items-center gap-2 text-primary hover:text-white">
            ...
            <div className="space-y-4">
              ...
              <h1 className="text-4xl md:text-7xl font-bold uppercase leading-[0.9]">{post.title}</h1>
              {post.subtitle && (
                <p className="text-xl text-muted-foreground font-light max-w-3xl">{post.subtitle}</p>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-8 pt-8 border-t border-white/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-sm font-bold uppercase">{post.author.name}</div>
                  <div className="text-[10px] text-muted-foreground uppercase">{post.author.role}</div>
                </div>
              </div>
        ...
        <div className="container mx-auto px-6 max-w-4xl space-y-16">
          {post.content?.map((section: any, idx: number) => {
            ...
            switch (section.type) {
              case "paragraph":
                return (
                  <motion.p key={key} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="text-xl text-muted-foreground leading-relaxed font-light">
                    {section.text}
                  </motion.p>
                );
              case "heading":
                return (
                  <motion.h2 key={key} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} className="text-4xl font-bold uppercase mt-8">
                    {section.text}
                  </motion.h2>
                );
              case "visual":
                return (
                  <motion.div key={key} className="space-y-4">
                    <GlassCard className="p-0 overflow-hidden aspect-video relative">
                      <Image src={section.imageUrl} alt={section.caption || ""} fill className="object-cover brightness-75" />
                    </GlassCard>
                    <p className="text-sm text-muted-foreground italic border-l-2 border-primary/30 pl-4">{section.caption}</p>
                  </motion.div>
                );
              case "takeaways":
                return (
                  <motion.div key={key}>
                    <GlassCard className="p-10 bg-accent/5 border-accent/20">
                      <h3 className="text-2xl font-bold uppercase mb-6 flex items-center gap-3">
                        <Target className="h-5 w-5" /> {section.title}
                      </h3>
